...
import {
  ...
  checkCaptchaClientTokenOnServer,
  logServerError,
} from "@/lib/server";
  ...
  return await Sentry.withServerActionInstrumentation(
    ...
    async (): Promise<FormResponse> => {
      try {
        ...
        if (!canSendForm) {
          return {
            message: "You reached your message limit. Please try again later.",
            hasError: true,
          };
        }
        const validatedData = contactFormSchema.parse(data);
        const { isValid, message } = await checkCaptchaClientTokenOnServer({
...
const _fnWithTimeout = addTimeoutToFunction({
  fn: _rawCreateMessageInDB,
  timeout: 5000,
});
const _fnWithTimeoutAndRetries = addRetriesToFunction({
  fn: _fnWithTimeout,
  loggingFn: logServerError,
  shouldStopTrying: (e) => isPrismaTransientError(e),
});
...
const _canUserSendFormWithTimeoutAndRetries = addRetriesToFunction({
  fn: _canUserSendFormWithTimeout,
  loggingFn: logServerError,
});
...
async function _canUserSendForm(ip: string | null): Promise<boolean> {
  if (process.env.NEXT_PUBLIC_PLAYWRIGHT_MODE === "yes") return true;
  if (!ip) return false;
