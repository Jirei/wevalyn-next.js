...
export async function checkCaptchaClientTokenOnServer({
  token,
  action,
}: {
  token: string;
  ...
  if (
    ...
    let internalErrorMessage =
      "Something unexpected went wrong while verifying the captcha client token";
    if (!parsedResponse.success)
      internalErrorMessage =
        "Recaptcha response came but the reCAPTCHA client token wasn't valid";
    else if (parsedResponse.score <= 0.5)
      internalErrorMessage =
        "Recaptcha response came but the score was lower than the thresold for bot detection clearance";
    else if (parsedResponse.action !== action)
      internalErrorMessage =
        "Recaptcha response came but the action used for the client captcha token didn't fit the one required";
    logServerError(new WrappingError(internalErrorMessage, parsedResponse));
    return {
      isValid: false,
      message:
        "Sorry but our security check found the activity unusual. Please try again later.",
    } as const;
  } else {
    return { isValid: true } as const;
  }
...
export function isPrismaTransientError(prismaError: unknown) {
  const zodSafeParseResult = z
    .object({ code: z.string() })
    .safeParse(prismaError);
...
const prismaTransientErrorCodes = [
  "P001",
  "P1002",
  "P1008",
  "P1011",
